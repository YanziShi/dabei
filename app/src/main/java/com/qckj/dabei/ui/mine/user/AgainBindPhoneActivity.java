...
public class AgainBindPhoneActivity extends BaseActivity {
    ...
    public static void startActivity(Activity context, int requestCode) {
        Intent intent = new Intent(context, AgainBindPhoneActivity.class);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_again_bind_phone);
        ViewInject.inject(this);
    }

    @OnClick({R.id.get_verification_code, R.id.sure_btn})
    ...
    private void bindPhone(String phoneNumber, String verCode) {
        ...
        userManager.againBindPhone(phoneNumber, verCode, new OnResultMessageListener() {
            @Override
            public void onResult(boolean isSuccess, String message) {
                dismissLoadingProgressDialog();
                if (isSuccess) {
                    setResult(RESULT_OK,new Intent().putExtra("phoneNumber",phoneNumber));
                    finish();
                } else {
                    showToast(message);
                }
